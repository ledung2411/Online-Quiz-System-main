...
exports.createStudentQuiz = async (req, res) => {
  try {
    ...
    if (students.length === 0 || quizzes.length === 0) {
      return res
        .status(400)
        .json({ message: "Class must have both students and quizzes" });
    }
    ...
    const studentQuizPromises = students.map(async (student) => {
      ...
      if (!studentQuiz) {
        ...
        const existingQuizIds = studentQuiz.quizzes.map((q) =>
          q.quizId.toString()
        );

        const newQuizzes = quizzes.filter(
          (quiz) => !existingQuizIds.includes(quiz._id.toString())
        );
    ...
    res
      .status(201)
      .json({ message: "Student quizzes created or updated successfully" });
  } catch (error) {
    console.error("Error creating or updating student quizzes:", error);
    res
      .status(500)
      .json({ message: "Error creating or updating student quizzes" });
  }
};
